...
import { parentService } from "@/api/parentService"; // Добавяме parentService за родителския профил
...
export function StudentGrades({ student }: { student?: User }) {
    ...
    useEffect(() => {
        ...
        // Динамично определяме кои функции от кое API да извикаме
        const fetchGradesAndSubjects = async () => {
            if (student && student.id) {
                // Ако компонентът е зареден от родителския профил с подадено дете
                return Promise.all([
                    parentService.getChildGrades(Number(student.id)),
                    parentService.getChildSubjects(Number(student.id))
                ]);
            } else {
                // Ако е зареден директно от профила на ученика
                return Promise.all([
    ...
    }, [me, student?.id]); // Добавяме student?.id като зависимост, за да реагира при смяна на детето от родителя
            ...
            <Card className="overflow-hidden border-primary/10">
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Тази таблица показва оценките по предмети и срокове.
                    </p>
                </CardHeader>
                <CardBody className="p-0">
                    {gradesBySubject.length === 0 ? (
                        <p className="py-6 text-center text-sm text-muted-foreground">
                            Все още няма нанесени оценки.
                        </p>
                    ) : (
