...
import { Plus, FileCheck2, ClipboardList, Sparkles, Search } from 'lucide-react'
...
import { classes, classById, subjectById, subjects, formatDate, type Homework, type User } from '@/lib/data'

export function TeacherHomework() {
  const { currentUser, homework, addHomework, addFeedback, users, selectedClassId, setSelectedClass } = useApp()
  ...
  const myHomework = useMemo(() => {
    ...
    return homework.filter(
      (h) => h.teacherId === currentUser.id && h.type === 'homework' && h.classIds.includes(selectedClassId),
    )
    ...
    <div className="space-y-6">
      <div className="rounded-[24px] border border-primary/15 bg-gradient-to-br from-primary/10 via-card to-brand-blue/10 p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Учителите добавят домашни задачи за избран клас и предмет.</p>
          </div>
          {canCreate && (
            <Button onClick={() => setCreating(true)} disabled={!selectedClassId}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="min-w-[220px] flex-1 sm:flex-none">
          <label className="mb-1.5 block text-sm font-medium">Избери клас</label>
          <select
      ...
      {creating && selectedClassId && (
        <CreateTaskDialog
          ...
          onCreate={(data) => {
            addHomework({ ...data, teacherId: currentUser!.id, type: 'homework', classIds: [selectedClassId] })
            setCreating(false)
          }}
        />
      )}
...
function CreateTaskDialog({ subjectIds, classId, onClose, onCreate }: { subjectIds: string[]; classId: string; onClose: () => void; onCreate: (data: { title: string; description: string; subjectId: string; dueDate: string }) => void }) {
  ...
  return (
    <Dialog open onClose={onClose} title="Добави домашна работа" className="max-w-xl">
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
          Домашната работа ще бъде добавена за клас {classById(classId)?.name ?? 'избрания клас'}.
        </div>

        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={onClose}>Връщане назад</Button>
          <Button onClick={() => { onCreate({ title, description, subjectId, dueDate }) }}>
            Добави домашно
          </Button>
        </div>
      </div>
