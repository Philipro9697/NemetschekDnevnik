...
import { Search, GraduationCap, Sparkles, Plus, ChevronDown, BookOpenCheck } from 'lucide-react'
...
import type { ClassDto, GradeDto, StudentInfoDto, SubjectDto } from '@/api/types'
...
export function TeacherGrades() {
  ...
  const [gradeTarget, setGradeTarget] = useState<{ studentId: number; section: GradeSection } | null>(null)
  ...
  useEffect(() => {
    teacherService.getClasses().then((data) => {
      setClassesList(data)
      if (data.length > 0) setSelectedClassId((prev) => prev ?? data[0].classId)
    }).catch((err) => console.error('Failed to load classes (are you logged in with a real account?):', err))
    teacherService.getSubjects().then((data) => {
      setSubjectOptions(data)
      if (data.length > 0) setSelectedSubjectId((prev) => prev ?? data[0].subjectId)
  ...
  useEffect(() => {
    ...
    teacherService.getStudents(selectedClassId).then(setStudents).catch((err) => console.error('Failed to load students:', err))
    teacherService.getGrades(selectedClassId).then(setGrades).catch((err) => console.error('Failed to load grades:', err))
  }, [selectedClassId])

  const selectedClass = classesList.find((c) => c.classId === selectedClassId) ?? null
  const selectedSubject = subjectOptions.find((s) => s.subjectId === selectedSubjectId) ?? null

  const filteredStudents = useMemo(() => {
    return query.trim()
      ? students.filter((s) => `${s.firstName} ${s.lastName}`.toLowerCase().includes(query.trim().toLowerCase()))
      : students
  }, [students, query])
  ...
  function openNoteDialog(studentId: number) {
    setNoteTarget({ studentId: String(studentId), subjectId: String(selectedSubjectId ?? '') })
    setNoteText('')
    setNoteKind('praise')
  }

  function openAbsenceDialog(studentId: number) {
    setAbsenceTarget({ studentId: String(studentId), subjectId: String(selectedSubjectId ?? '') })
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {classesList.map((klass) => (
              <Button key={klass.classId} variant={selectedClassId === klass.classId ? 'default' : 'outline'} size="sm" onClick={() => setSelectedClassId(klass.classId)}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card/80 p-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <GraduationCap className="size-4 text-primary" /> Преглед за {selectedClass ? `${selectedClass.classGrade}${selectedClass.classLetter}` : 'клас'}
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <button type="button" onClick={() => setSubjectMenuOpen((prev) => !prev)} className="inline-flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-2 text-sm font-medium">
              <BookOpenCheck className="size-4 text-primary" /> Оценки - {selectedSubject?.subjectName ?? 'Предмет'}
            ...
            {subjectMenuOpen && (
              <div className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-border bg-card p-2 shadow-lg">
                {subjectOptions.map((subject) => (
                  <button key={subject.subjectId} type="button" onClick={() => { setSelectedSubjectId(subject.subjectId); setSubjectMenuOpen(false) }} className="flex w-full items-start rounded-lg px-3 py-2 text-left text-sm hover:bg-muted">
          ...
          <div className="text-sm text-muted-foreground">Общо ученици: {filteredStudents.length}</div>
        </div>
      </div>
      ...
      <Card className="overflow-hidden border-primary/10">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Тази таблица показва оценките на всеки ученик по срокове за избрания предмет.</p>
        </CardHeader>
        <CardBody className="p-0">
          {filteredStudents.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">Няма ученици в този клас.</p>
          ) : (
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    ...
                    <th className="w-[130px] border-l border-border/70 px-4 py-3">Бележки</th>
                    <th className="w-[130px] border-l border-border/70 px-4 py-3">Отсъствия</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredStudents.map((student) => {
                    const studentGrades = grades.filter((g) => g.studentId === student.studentId && g.subjectId === selectedSubjectId)
                    const studentIdStr = String(student.studentId)
                    const studentNotes = notes.filter((n) => n.studentId === studentIdStr && n.subjectId === String(selectedSubjectId ?? ''))
                    const studentAbsences = absences.filter((a) => a.studentId === studentIdStr && a.subjectId === String(selectedSubjectId ?? ''))
                    return (
                      <tr key={student.studentId} className="border-t border-border/70">
                        <td className="px-4 py-3 align-top">
                          <div className="font-medium text-foreground">{student.firstName} {student.lastName}</div>
                          <div className="mt-1 text-xs text-muted-foreground">
                            {studentGrades.length} оценка{studentGrades.length === 1 ? '' : 'и'}
                          </div>
                        </td>
                        {gradeSections.map((section) => {
                          const sectionGrades = studentGrades.filter((g) => decodeComment(g.comment).section === section.key)
                          return (
                            <td key={section.key} className="border-l border-border/70 px-4 py-3 align-top">
                              <div className="flex flex-wrap items-center gap-1">
                                ...
                                <button type="button" onClick={() => openGradeDialog(student.studentId, section.key)} className="rounded-md p-0.5 text-primary hover:bg-muted">
                        ...
                        <td className="border-l border-border/70 px-4 py-3 align-top">
                          <div className="flex items-center justify-center gap-1.5 rounded-lg border border-border/70 bg-muted/20 px-2 py-1.5">
                            <button type="button" onClick={() => setNotesPopupTarget({ studentId: studentIdStr, subjectId: String(selectedSubjectId ?? '') })} className="rounded-md px-2 py-1 text-sm font-semibold text-foreground hover:bg-background">
                              {studentNotes.length}
                            </button>
                            <button type="button" onClick={() => openNoteDialog(student.studentId)} className="rounded-md p-1 text-primary hover:bg-background">
                              <Plus className="size-3.5" />
                            </button>
                          </div>
                        </td>
                        <td className="border-l border-border/70 px-4 py-3 align-top">
                          <div className="flex items-center justify-center gap-1.5 rounded-lg border border-border/70 bg-muted/20 px-2 py-1.5">
                            <button type="button" onClick={() => setAbsencesPopupTarget({ studentId: studentIdStr, subjectId: String(selectedSubjectId ?? '') })} className="rounded-md px-2 py-1 text-sm font-semibold text-danger hover:bg-background">
                              {studentAbsences.length}
                            </button>
                            <button type="button" onClick={() => openAbsenceDialog(student.studentId)} className="rounded-md p-1 text-danger hover:bg-background">
                              <Plus className="size-3.5" />
                            </button>
                          </div>
                        </td>
      ...
      <Dialog open={Boolean(gradeTarget)} onClose={() => setGradeTarget(null)} title="Добави оценка">
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
            <div className="font-semibold text-foreground">{selectedSubject?.subjectName ?? 'Предмет'}</div>
            <div className="mt-1">{gradeTarget ? gradeSections.find((s) => s.key === gradeTarget.section)?.label : ''}</div>
          </div>
          <div>
      ...
      <Dialog open={Boolean(noteTarget)} onClose={() => setNoteTarget(null)} title="Добави бележка">
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
            {noteTarget && <div className="font-semibold text-foreground">{studentName(noteTarget.studentId)}</div>}
          </div>
      ...
      <Dialog open={Boolean(absenceTarget)} onClose={() => setAbsenceTarget(null)} title="Добави отсъствие">
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
            {absenceTarget && <div className="font-semibold text-foreground">{studentName(absenceTarget.studentId)}</div>}
          </div>
        ...
        {selectedGrade && (
          <div className="space-y-3 text-sm">
            <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 px-3 py-2">
              <div className="font-semibold">{selectedGrade.studentFirstName} {selectedGrade.studentLastName}</div>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-3">
              ...
              <div>{gradeSections.find((s) => s.key === decodeComment(selectedGrade.comment).section)?.label}</div>
            </div>
            <div className="rounded-lg border border-border bg-muted/30 p-3">
            ...
            {decodeComment(selectedGrade.comment).text && (
              <div className="rounded-lg border border-border bg-muted/30 p-3 text-muted-foreground">{decodeComment(selectedGrade.comment).text}</div>
            )}
          </div>
        )}
      </Dialog>
